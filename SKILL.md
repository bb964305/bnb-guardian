...
- 状态文件: `memory/binance-state.json`
- 交易记录: `memory/binance-trades.json`
- 学习日志: `memory/binance-lessons.json`
- 交易日记: `memory/binance-diary.md`

## 触发条件

以下任一条件满足时激活本 Skill：
1. skill 首次安装后自动运行
2. 用户提到 "币安"、"交易"、"行情"、"BNB"、"持仓"、"分析"、"菜单"
3. cron 定时触发（5分钟监控循环）
4. 用户说 "暂停" 暂停自动交易，"恢复" 恢复
5. 用户输入数字 1-9 时匹配主菜单
...
检查 `memory/binance-state.json` 是否存在且 `initialized=true`。如果是，跳到第一步。

### 0.1 凭证检查

检查 `vault/binance-api.json` 是否存在。

**如果存在** → 读取 api_key 和 api_secret，跳到 0.3 验证。

**如果不存在** → 发送：
```
🔑 BNB Guardian 需要你的币安 API Key

还没有？3 步搞定：
1. 币安 App → 更多 → API 管理 → 创建 API
2. 权限：✅读取 ✅现货 ✅合约 ❌提现（绝对不开！）
3. 把 API Key 和 Secret 发给我（分两条）

已有的话直接发过来。
```

收到后保存到 `vault/binance-api.json`：
```json
{"api_key": "xxx", "api_secret": "xxx", "created": "日期"}
```

### 0.2 风险确认（一句话）

```
⚠️ 加密交易有风险，AI 分析不构成投资建议。回复 "OK" 继续。
```

### 0.3 验证凭证 + 拉取账户数据

立即调用三个账户接口，验证凭证的同时拉取真实数据：
...
QUERY="timestamp=$TS&recvWindow=5000"
SIG=$(echo -n "$QUERY" | openssl dgst -sha256 -hmac "$API_SECRET" | awk '{print $NF}')
curl -s -H "X-MBX-APIKEY: $API_KEY" "https://fapi.binance.com/fapi/v2/balance?${QUERY}&signature=${SIG}"
...
如果 401/403 → 提示用户检查 API Key，重新输入。

### 0.4 首次展示：账户全景 + 市场快照

验证成功后，**立刻**把真实数据推送给用户（这是用户看到的第一个东西）：

```
🦞 BNB Guardian 已连接你的币安账户！
...
📈 合约账户
  ...
  可用保证金: $X,XXX
  📊 持仓:
    BNBUSDT 多 Xx | 入场 $XXX | 盈亏 +X.X%
    ETHUSDT 空 Xx | 入场 $X,XXX | 盈亏 -X.X%
  （无持仓则显示"暂无持仓"）
...
💎 BNB/USDT: $XXX.XX (24h +X.X%)
₿ BTC/USDT: $XX,XXX (24h +X.X%)
Ξ ETH/USDT: $X,XXX (24h -X.X%)
...
```
4️⃣ 关注币种（默认 BNB/BTC/ETH/SOL）
   直接回复币种，用逗号分隔。如：BNB,BTC,ETH,SOL,DOGE
   回复 "默认" 使用默认列表
```

每一步都是单独的消息，等用户回复后再下一步。不要一次性发完。
...
```
🦞 BNB Guardian 就绪！回复数字使用功能：

━━━━━━━ 📋 主菜单 ━━━━━━━

1️⃣  💰 账户总览 — 三账户资产 + 持仓 + 盈亏
2️⃣  🔍 行情分析 — BNB 综合技术分析（MA/RSI/MACD/评分）
3️⃣  📊 监控面板 — 异动检测 + 深度数据 + 市场情绪
4️⃣  💹 交易操作 — 买入/卖出/开多/开空/挂单
5️⃣  🎯 策略中心 — 网格/套利/趋势/定投/回测
6️⃣  🛡️ 风控状态 — 止损/熔断/仓位/风险仪表盘
7️⃣  📈 收益报告 — 日报/周报/月报/历史绩效
8️⃣  🧠 学习日志 — 复盘记录/策略评估/进化历史
9️⃣  ⚙️ 设置 — 偏好/推送/关注币种/交易模式
0️⃣  📢 币安广场 — 自动发帖/互动/涨粉

💡 也可以直接说话：
  "BNB 怎么样" "买 0.5 BNB" "暂停交易" "今日报告"
...
| 输入 | 动作 |
|------|------|
| 1 | 执行 → 账户总览（拉三账户数据，格式化展示） |
| 2 | 执行 → 行情分析（对默认币种或上次分析的币种做完整技术分析） |
| 3 | 执行 → 监控面板（当前异动 + 深度数据 + 情绪 + 资金费率） |
| 4 | 推送 → 交易子菜单 |
| 5 | 推送 → 策略子菜单 |
| 6 | 执行 → 风控仪表盘 |
| 7 | 推送 → 报告子菜单 |
...
请回复数字，或直接说 "买 0.5 BNB" "BNB 开多 3x"
```

用户选择后引导输入参数：
- 选 1（买入）→ "请输入：币种 数量（如：BNB 0.5）"
- 选 4（开多）→ "请输入：币种 杠杆 数量（如：BNB 5x 1）"
...
1️⃣  网格交易 — 设置价格区间自动网格
2️⃣  趋势跟踪 — AI 自动判断趋势方向
3️⃣  费率套利 — 资金费率异常时自动套利
4️⃣  定投计划 — 定时定额买入
5️⃣  回测 — 用历史数据验证策略
6️⃣  当前运行中的策略
7️⃣  暂停所有策略
...
📦 现货账户
  币种     数量        价值        24h
  BNB     10.500    $6,746     +2.3% 📈
  USDT    2,350.00  $2,350     —
  BTC     0.0500    $4,250     -0.5% 📉
  ETH     1.200     $2,400     +1.2% 📈
  💰 小计: $15,746
...
🔥 异动检测（最近1小时）
  🔴 PEPE/USDT  +18.5%  量↑5.2x  新高！
  🟡 SOL/USDT   +5.3%   量↑2.1x  突破MA99
  🟡 ARB/USDT   -4.2%   量↑1.8x  跌破支撑
...
无论用户是否操作，以下内容按频率自动推送：

**每小时推送（如果 notification_frequency = realtime 或 hourly）：**
```
📊 整点快报 (HH:00)
BNB $645 +1.2% | BTC $85.2K -0.3% | ETH $2.03K +0.8%
持仓盈亏: +$290 (+1.24%) | 风控: ✅
```

**每日推送（8:00 早报 + 20:00 晚报）：**

早报：
```
🌅 早安！BNB Guardian 日报 (日期)
━━━━━━━━━━━━━━━━━━━━━
...
BNBUSDT 15分钟跌 -5.2%
当前: $612 | 成交量: ↑3.8x
你的持仓: 多5x 浮亏 -$180 (-2.86%)
风控: 距止损 $600 还有 $12

回复 1 查看持仓 | 回复 "平仓 BNB" 平仓
```

**交易执行推送：**
```
⚡ 交易执行
BUY 0.5 BNB @ $645.20 (市价)
花费: $322.60 USDT
手续费: $0.32 (0.1%)
执行策略: 趋势跟踪
入场评分: 68/100
